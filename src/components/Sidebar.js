...
import { ExpandLess, ExpandMore, AccountBox } from "@material-ui/icons";
...
const LoginUserIcon = styled(AccountBox)`
...
function SidebarLink({ name, to, badge, onClose }) {
    return (
        <Link
            button
            dense
...
function Sidebar({ classes, staticContext, location, ...rest }) {
    ...
    return (
        <Drawer variant="permanent" {...rest}>
            <Brand className="sidebarBrand">
                <img alt="MEIDEN" height="34px" src="/static/img/brands/mdx_w_logo.png" />
            </Brand>
            <Scrollbar
                containerRef={el => (scrollBarRef.current = el)}
                onScrollY={(container) => setCurrentScroll(container.scrollTop)}
            >
                <List disablePadding>
                    <Items>
